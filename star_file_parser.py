...
__author__ = 'Ahmad Abdelzaher Zaki Khalifa'
...
class Table:
    ...
    #Ahmad
    def _updateTuple(self, NamedTuple, nameOfNamedTuple, colName, colValue):
        NamedTuple_asdict = NamedTuple._asdict()
        NamedTuple_asdict[colName]= colValue
        updated_NamedTuple = namedtuple(nameOfNamedTuple, NamedTuple_asdict.keys())(**NamedTuple_asdict)
        return updated_NamedTuple
        
    #Ahmad
    def _changeColumValue(self, colName, colValue):
        if colName not in tableIn._columns:
            raise Exception("Not existing column: %s" % col_name)
        for i in range(len(self._rows)):
            self._rows[i] = self._updateTuple(self._rows[i], 'Row', colName, colValue)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Script to manipulate metadata of STAR files.")
    #Ahmad
    add = parser.add_argument  # shortcut
    add("input", help="Input STAR file ", nargs='?', default="")
    
    add("--output",help="Output STAR file. If not provided, the input STAR file will be printed on screen.", nargs='?')
    add("--limit", type=int, default=0,help="Limit the number of rows processed, useful for testing.")

    add('--change_value', nargs=2, help='[column value] that you want to change.')
    
    add('--add_column_and_value', nargs=2, help='[column value] that you want to add.')
    
    add('--add_column_and_value_from_column', nargs=3, help='[column existing column value] that you want to add.')
    ...
    #Ahmad change row values of tableIn
    if args.change_value:
        col_name, col_value = args.change_value
        if col_name not in tableIn._columns:
            raise Exception("Not existing column: %s" % col_name)
        tableIn._changeColumValue(col_name, col_value)
 
    #Ahmad add column and value
    if args.add_column_and_value:
        col_name, col_value = args.add_column_and_value
        if col_value =='random':
            col_value = random.uniform(0,360)
        tableIn._addColumn(col_name)
        tableIn._changeColumValue(col_name, col_value)
    
    #Ahmad add column and calculate value from another column
    if args.add_column_and_value_from_column:
        col1, col2, col_value = args.add_column_and_value_from_column
        if col2 not in tableIn._columns:
            raise Exception("Not existing column: %s" % col2 )
        tableIn._addColumn(col1)
        col_value = float(tableIn.getColumnValues('%s' %col2)[0])+float(col_value)
        tableIn._changeColumValue(col1, col_value)
